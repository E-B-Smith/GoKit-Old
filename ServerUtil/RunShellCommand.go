...
func RunShellCommand(shellCommand string, parameters []string, standardIn []byte)    (standardOut []byte, standardError []byte, error error) {
    ...
    if error != nil {
        log.Error("Can't find path for '%s': %v.", shellCommand, error)
        return nil, nil, error
    }
    log.Debug("     Path: %s.", path)
    log.Debug("StdIn Len: %d.", len(standardIn))
    ...
    if  len(standardIn) > 0 {
        ...
        if error != nil {
            log.Error("Can't open input pipe: %v.", error)
            return nil, nil, error
        }
    ...
    error = command.Start()
    if error != nil {
        log.Error("Error starting '%s': %v.", path, error)
        return nil, nil, error
    }
